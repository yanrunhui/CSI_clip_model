...
def read_prediction_means(path: Path) -> dict[str, dict[str, float]]:
    ...
    has_los_probability = "pred_los_probability" in rows[0]
    has_los_status = "pred_los_status" in rows[0]
    grouped: dict[str, list[float]] = {}
    grouped_los_probability: dict[str, list[float]] = {}
    grouped_los_votes: dict[str, list[float]] = {}
    for row in rows:
        ...
        if has_los_probability:
            los_probability = float(row["pred_los_probability"])
            if not math.isfinite(los_probability) or not 0.0 <= los_probability <= 1.0:
                raise ValueError(
                    f"Invalid pred_los_probability at timestamp {row['timestamp']}"
                )
            grouped_los_probability.setdefault(str(row["timestamp"]), []).append(
                los_probability
            )
        if has_los_status:
            los_status = str(row["pred_los_status"]).lower()
            if los_status not in {"los", "nlos"}:
                raise ValueError(
                    f"Invalid pred_los_status={los_status!r} at timestamp "
                    f"{row['timestamp']}"
                )
            grouped_los_votes.setdefault(str(row["timestamp"]), []).append(
                1.0 if los_status == "los" else 0.0
            )
    ...
    if has_los_probability:
        for timestamp, values in grouped_los_probability.items():
            mean_probability = float(np.mean(values))
            result[timestamp]["predicted_los_probability_mean"] = mean_probability
    if has_los_status:
        for timestamp, votes in grouped_los_votes.items():
            los_vote_fraction = float(np.mean(votes))
            if math.isclose(los_vote_fraction, 0.5) and has_los_probability:
                is_los = (
                    float(result[timestamp]["predicted_los_probability_mean"]) >= 0.5
                )
            else:
                is_los = los_vote_fraction > 0.5
            result[timestamp].update(
                {
                    "predicted_los_vote_fraction": los_vote_fraction,
                    "predicted_los_status": "los" if is_los else "nlos",
                    "predicted_los_vote_confidence": max(
                        los_vote_fraction, 1.0 - los_vote_fraction
                    ),
                }
            )
    return result
...
def main() -> None:
    ...
    predicted_group_rows: list[dict[str, object]] = []
    if detail_rows and "predicted_los_status" in detail_rows[0]:
        primary_rows = [
            row
            for row in detail_rows
            if math.isclose(
                float(row["threshold_db_below_peak"]), args.primary_threshold_db
            )
        ]
        for status in ("los", "nlos"):
            selected = [
                row for row in primary_rows if row["predicted_los_status"] == status
            ]
            if not selected:
                continue
            reference = np.asarray(
                [float(row["reference_delay_spread_ns"]) for row in selected],
                dtype=np.float64,
            )
            prediction = np.asarray(
                [float(row["prediction_mean_ns"]) for row in selected],
                dtype=np.float64,
            )
            predicted_group_rows.append(
                {
                    "grouping": "model_predicted_not_ground_truth",
                    "predicted_los_status": status,
                    "primary_threshold_db_below_peak": args.primary_threshold_db,
                    "mean_predicted_los_probability": float(
                        np.mean(
                            [
                                float(row["predicted_los_probability_mean"])
                                for row in selected
                            ]
                        )
                    ),
    ...
    if predicted_group_rows:
        write_csv(
            output_dir / "delay_spread_predicted_los_group_summary.csv",
        ...
        json.dump(
            {
                ...
                "predicted_los_grouping": {
                    "status": "model_predicted_groups_not_ground_truth",
                    "groups": predicted_group_rows,
                },
            },
            stream,
            indent=2,
    ...
    if predicted_group_rows:
        print("Predicted LoS/NLoS group comparison (not ground truth):")
        for row in predicted_group_rows:
            print(
                f"  {str(row['predicted_los_status']).upper()}: "
                f"points={row['mat_count']} "
                f"reference_mean={row['reference_mean_ns']:.3f} ns "
                f"prediction_mean={row['prediction_mean_ns']:.3f} ns "
                f"MAE={row['mae_ns']:.3f} ns "
                f"bias={row['bias_ns']:.3f} ns "
