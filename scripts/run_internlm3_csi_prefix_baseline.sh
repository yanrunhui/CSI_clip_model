...
case "$MODE" in
  smoke)
    STEPS="${STEPS:-5}"
    LIMIT="${LIMIT:-2}"
    OUT="${OUT:-artifacts/internlm3_8b_csi_prefix/smoke_2samples_5steps}"
    ;;
  full)
    STEPS="${STEPS:-500}"
    LIMIT="${LIMIT:-1000}"
    OUT="${OUT:-artifacts/internlm3_8b_csi_prefix/seed_0}"
    ;;
  *)
...
python scripts/validate_internlm3_csi_prefix.py --model-path "$MODEL"

python scripts/train_internlm3_csi_prefix.py \
  --model-path "$MODEL" \
  --train-data "$TRAIN" \
  --output-dir "$OUT" \
...
test -f "$OUT/internlm_adapter/adapter_config.json" || {
  echo "training did not produce the InternLM3 LoRA adapter" >&2
  exit 1
}

python scripts/evaluate_internlm3_csi_prefix.py \
  --model-path "$MODEL" \
  --mapping-checkpoint "$OUT/mapping_final.pt" \
  --adapter-path "$OUT/internlm_adapter" \
  --data-path "$TEST" \
...
if float(summary.get("parse_success_rate", -1.0)) != 1.0:
    print("internlm3_json_parse_failures:")
...
print(
    "internlm3_strict_json_format_rate="
    + str(summary.get("strict_json_format_rate"))
)
print("internlm3_generation_validation=passed")
PY

touch "$OUT/BENCHMARK_COMPLETE"
echo "internlm3_${MODE}_complete=$OUT"
