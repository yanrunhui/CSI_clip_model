...
def summarize(
    ...
    fieldnames = ["ablation", "seed", "kind", "purpose"]
    for row in rows:
        for key in row:
    ...
    metric_names = [name for name in fieldnames if name not in {"ablation", "seed", "kind", "purpose"}]
    for spec in specs:
    ...
    aggregate_fields = ["ablation", "seeds", "kind", "purpose"]
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=("all",),
        help="Ablations to include, or all. Use --list to print names.",
    )
