...
from scripts.train_qwen_csi_prefix import (  # noqa: E402
    inherit_controlled_comparison_args,
...
def load_trainable_internlm3(args: argparse.Namespace, dtype: torch.dtype):
    ...
    if args.use_lora:
        ...
        if args.load_in_4bit and not args.skip_kbit_preparation:
            ...
            print(
                "internlm3_kbit_preparation=skipped "
        ...
        lora_config = LoraConfig(
            ...
            target_modules=internlm3_lora_target_modules(model),
            bias="none",
        )
    ...
    set_internlm3_use_cache(model, False)
    return model


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Train an explicit CSIEncoder-to-InternLM3 soft-prefix baseline."
    )
    parser.add_argument("--model-path", required=True)
    parser.add_argument("--train-data", required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--csi-checkpoint")
    parser.add_argument("--controlled-config-from")
    ...
    parser.add_argument("--skip-kbit-preparation", action="store_true")
...
def main() -> None:
    ...
    model = load_trainable_internlm3(args, dtype)
    ...
    trainable.extend(parameter for parameter in model.parameters() if parameter.requires_grad)
    if not trainable:
    ...
    total_steps = args.max_steps if args.max_steps > 0 else max(args.epochs * updates_per_epoch, 1)
    ...
    metadata = {
        ...
        "architecture": "CSIEncoder -> MLP projector -> InternLM3 soft prefix",
        "mapper_config": mapper_config.__dict__,
        "csi_checkpoint_load": csi_load_metadata,
        "train_samples": len(dataset),
        "language_model": args.model_path,
        "language_model_architectures": list(getattr(model.config, "architectures", None) or []),
        "language_model_hidden_size": hidden_size,
        "use_lora": args.use_lora,
        "controlled_comparison": controlled_comparison,
    }
    print("internlm3_csi_prefix_training_metadata=" + json.dumps(metadata, sort_keys=True))
    print(f"trainable_parameters={sum(parameter.numel() for parameter in trainable)}")
    ...
    for epoch in range(args.epochs if args.max_steps <= 0 else 10**9):
        for batch in loader:
            ...
            with autocast_context():
                ...
                if not bool(torch.isfinite(output.loss)):
                    raise FloatingPointError(
                        f"Non-finite InternLM3 loss before step {global_step + 1}: "
                        f"{output.loss.item()}."
                    )
            ...
            if not bool(torch.isfinite(grad_norm)):
                raise FloatingPointError(
                    f"Non-finite InternLM3 gradient norm before step {global_step + 1}."
                )
    ...
    adapter_path = None
    if args.use_lora:
        adapter_path = output_dir / "internlm_adapter"
    ...
    print(f"saved_internlm3_csi_prefix_mapping={final_path}")
    if adapter_path:
        print(f"saved_internlm3_adapter={adapter_path}")
